...
export async function realReadExcel(
  ...
  // ── Parse via DocumentService (S14-001) ──────────────────────────────────
  ctx.logger.info('Parsing Excel file via DocumentService...');
  try {
    const result = documentService
      ...
      : fallbackParseExcel(fileBuffer, ctx);   // graceful fallback if not injected yet
...
// ── Inline fallback (used when DocumentService not yet injected) ─────────────
// Keeps backward-compat during migration period.

import * as XLSX from 'xlsx';
import type { ExcelRow } from '../../document/document.service';
...
function fallbackParseExcel(buffer: Buffer, ctx: NodeContext) {
  ...
  return { sheetName, sheets: workbook.SheetNames, headers, rows, rowCount: rows.length };
}
