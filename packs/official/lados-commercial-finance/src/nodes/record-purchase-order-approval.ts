/**
 * lados.finance.record_purchase_order_approval — Phase 21 S5 (Wave 3)
 *
 * Same contract as record-invoice-approval.ts, targeting a purchase_order
 * resource. Never fabricates `approvedBy` — mirrors
 ...
 *   resourceId  — the purchase_order resource id (required)
...
export interface RecordPurchaseOrderApprovalServices {
  updateService?: IUpdateResourceService;
  transitionService?: ITransitionResourceService;
}

export async function recordPurchaseOrderApproval(
  ctx: NodeContext,
  services: RecordPurchaseOrderApprovalServices = {},
  ...
  if (!resourceId) {
    return {
      status: 'failure',
      outputs: { approval: null },
      error: { code: 'MISSING_INPUT', message: 'lados.finance.record_purchase_order_approval: resourceId is required' },
    };
  }
  if (!decision) {
    return {
      status: 'failure',
      outputs: { approval: null },
      error: { code: 'MISSING_INPUT', message: 'lados.finance.record_purchase_order_approval: decision ("approved"/"rejected") is required' },
    };
  }
  if (!approvedBy) {
    return {
  ...
  if (!ctx.organizationId) {
    return {
      status: 'failure',
      outputs: { approval: null },
      error: { code: 'MISSING_CONTEXT', message: 'lados.finance.record_purchase_order_approval: organizationId missing from execution context' },
    };
  }
  ...
  ctx.logger.info(`lados.finance.record_purchase_order_approval → po:${resourceId} decision:${decision} by:${approvedBy}`);
  ...
  try {
    ...
    if (result.approvalRequired) {
      return {
        ...
        pause: {
          title: `Approve purchase order status change to "${toState}"`,
          description: notes ?? 'Purchase order approval requires a further approval step',
          assigneeRole: 'owner',
          context: { approvalTaskId: result.approvalTaskId },
        },
        summary: `Paused: PO ${resourceId} status change to "${toState}" requires approval`,
      };
    }
    ...
    return {
      ...
      summary: `Purchase order ${resourceId} ${decision} by ${approvedBy}`,
    };
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    ctx.logger.error(`lados.finance.record_purchase_order_approval failed: ${message}`);
